...
from granular_ball.v3_granular_ball_自适应论文方法 import GranularBallV3
...
class GranularThreeWayClassifierV3:
    ...
    def __init__(self,
                 ...
                 alpha: float = 0.7,
                 beta: float = 0.3):
        self.gb_model = GranularBallV3(min_purity=min_purity)
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> Dict:
        ...
        for ball_info in self.gb_model.balls_:
            # 兼容新旧版本的粒球信息
            if len(ball_info) == 4:  # 新版本 (center, radius, indices, level)
                center, radius, indices, _ = ball_info
            else:  # 旧版本 (center, radius, indices)
                center, radius, indices = ball_info
